...
> **Status (2026-06-22): DEFERRED — not built.** Custom domains is a Growth/Premium branding perk, not a functional requirement (diners reach menus by QR/NFC, never by typing the URL). Decision: build it when a paying Growth/Premium customer actually requests their own domain. Already in place: the `tenants.custom_domain` column (unique, guarded), `getTenantByCustomDomain(host)` in `src/lib/tenant.ts` (currently unused), and plan gating (`allowsCustomDomain`, growth+premium). Still needed to ship it: (1) host-routing middleware (there is no `middleware.ts` yet — see Tenant resolution below; rewrite non-`platodigital.io` hosts to the matched tenant's menu), (2) Vercel Domains API integration to add + verify a domain and auto-provision its SSL (`VERCEL_TOKEN` is already in env), (3) an admin UI on `/admin/tenants/[slug]` to set the domain and show verification status. Gotcha: the restaurant's own DNS governs this; if they use Cloudflare with the proxy on (orange cloud), it can clash with Vercel's SSL verification — instruct "DNS only" (grey cloud).
