# Client Service Agreement, Plato

> ⚠️ **DRAFT, NOT LEGAL ADVICE.** Generated from the project specs (finance.md, operations.md,
> architecture.md) as a starting point for your attorney. **Must be reviewed and approved by
> a qualified attorney (Aruba law) before signing.** Fill every `[bracket]`.

This Agreement is between **Plato**, a service of **GMS Innovations** ("Plato"), a sole
proprietorship registered in Aruba, and **[Restaurant legal name]** of **[address]**
("Client"), effective **[DATE]**.

## 1. Services
Plato provides a done-for-you video-menu service:
- On-site **capture** of food photos and short videos to Plato's standard.
- **Menu build** and publishing at `platodigital.io/[slug]` (or a custom domain on eligible
  plans), with a USD/AWG price toggle and EN/ES content.
- A **dashboard** for the Client's quick edits (sold-out, price, specials).
- **QR/NFC** materials and tracked links; **analytics** (views, scans, top dishes).
- **Support** and change requests, and re-shoots per the plan cadence below.

## 2. Plan and inclusions
- **Selected plan:** ☐ Starter ☐ Growth ☐ Premium, per the current price list.
- **Item cap, hardware, custom domain, and re-shoot cadence** are per the plan:
  Starter pays per re-shoot; Growth includes one re-shoot per year; Premium includes a
  quarterly re-shoot.

## 3. Term and renewal
- Initial term: **[monthly / 12 months]**, starting on the publish date.
- Auto-renews for successive periods unless either party gives **[30] days'** written
  notice before renewal.

## 4. Fees and payment
- **Monthly fee:** $[ ] · **One-time setup fee:** $[ ] · **Add-ons:** [ ].
- **Optional rented tablet:** $[ ]/month, refundable deposit $[ ], minimum term
  **[6-12] months**; the device remains Plato's property (kiosk-locked to the Client's menu).
- Billed in **USD** by invoice (bank transfer or in person); due within **[14] days**.
- Late payment may lead to a short grace period and then **suspension** of the page.
- Client is responsible for applicable taxes (incl. Aruba BBO/turnover tax).

## 5. Client responsibilities
Provide accurate menu information, prices, hours, location, and access for the capture
visit; designate a contact; review and approve translated (ES) drafts before publish; and
keep account credentials secure.

## 6. Content and media license
The Client retains ownership of its brand, dish names, descriptions, and prices. Photos and
videos captured by Plato are licensed to the Client for use on the Plato menu during the
term. Plato may show a discreet "Powered by Plato" link and, on eligible plans, feature the
Client in the public Discover directory.

## 7. Hardware
QR stickers/stands, NFC tags, and decals provided per the plan. Rented tablets remain
Plato's property; on cancellation the Client returns the device in good condition or the
deposit/damage clause applies.

## 8. Data protection
Each party complies with applicable data-protection law. Plato's handling of data is
described in the Privacy Policy. Diner analytics are cookieless and contain no personal data.

## 9. Termination and offboarding
On cancellation or non-renewal: the page is unpublished after the paid period ends; the
Client may request an **export** of its menu content; any rented tablet and Plato-owned
hardware are returned; and the Client's data is retained per the Privacy Policy retention
period and then deleted.

## 10. Warranties, liability, and indemnity
The service is provided with reasonable skill and care but without further warranty. To the
extent permitted by law, neither party is liable for indirect or consequential loss, and
Plato's aggregate liability is limited to fees paid in the prior **[3]** months. Each party
indemnifies the other for claims arising from its own breach or unlawful content.

## 11. Confidentiality
Each party keeps the other's non-public information confidential and uses it only to perform
this Agreement.

## 12. Governing law
This Agreement is governed by the laws of **Aruba**; disputes are subject to the courts of
Aruba.

## 13. Signatures
| Plato | Client |
|---|---|
| Name: ________________ | Name: ________________ |
| Title: _______________ | Title: _______________ |
| Date: ________________ | Date: ________________ |
| Signature: ___________ | Signature: ___________ |
