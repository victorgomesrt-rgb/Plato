...
export default async function AdminRequestsPage() {
  ...
  const [{ data: crs }, { data: hws }, { data: tenants }] = await Promise.all([
    svc.from("change_requests").select("id, kind, message, status, created_at, owner_reply, tenant_id, tenants(name, slug)").order("created_at", { ascending: false }).limit(100).returns<ChangeReq[]>(),
